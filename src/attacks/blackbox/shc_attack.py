"""
Implements the shc based attack
"""
...
from utils.compute_fcts import lp_step, linf_step, sign


class SHCAttack(BlackBoxAttack):
    """
    Sign Attack
    """
    ...
    def _suggest(self, xs_t, loss_fct, metric_fct):
        ...
        bxs_t = self.xo_t
        est_deriv = (loss_fct(fxs_t.cpu().numpy()) - loss_fct(bxs_t.cpu().numpy())) / self.epsilon
        switch_flag = np.array([i for i, val in enumerate(est_deriv < self.best_est_deriv) if val])
