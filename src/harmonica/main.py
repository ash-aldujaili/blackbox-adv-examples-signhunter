...
parser.add_argument('-query', type=str, default='f1', help='which function to use')
opt = parser.parse_args()

if opt.query == 'f1':
    samplings.query = samplings.f1
else:
    samplings.query = samplings.f2
...
end_time = time.time()
time_per_query = (end_time - start_time) / samplings.num_queries
printSeparator()
print('best answer : ', bestAnswer)
print('number of queries: ', samplings.num_queries)
print('Time per query {:4f}s'.format(time_per_query))
